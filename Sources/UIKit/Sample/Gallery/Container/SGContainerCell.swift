...
extension DTB {
    ...
    /// 容器 cell：左 key + 右 `ContainerView` 预览。
    final class SGContainerCell: DTB.BaseTableViewCell {

        func config(key: String, style: DTB.ContainerStyle) {
            keyLabel.text = key
        ...
        private lazy var preview = DTB.ContainerView()
    }
}
